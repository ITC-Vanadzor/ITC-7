...
Create view account_extend AS (select user.id as id, user.username as name, user.email as email, user.password as pass, MAX(IF(`f_id` = 1, f_value, "...")) surname, MAX(IF(`f_id` = 2, f_value, "...")) birthday, MAX(IF(`f_id` = 3, f_value, "...")) image from (account join user) where (account.u_id=user.id) GROUP BY id );
