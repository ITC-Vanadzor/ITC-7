...
std::cout << "Insert number of tops " << std::endl;
std::cin >> number_of_tops;
std::cout << "Insert coordinates of first top " << std::endl;
std::cin >> x1;
std::cin >> y1;
std::cout << "Insert coordinates of second top " << std::endl;
std::cin >> x2;
std::cin >> y2;
std::cout << "Insert coordinates of third top " << std::endl;
std::cin >> x3;
std::cin >> y3;
...
if (vector_product > 0)
{
 int temp = 1;
	while(temp <= (number_of_tops - 3))
		{
		std::cout << "Import coordinates of next top " << std::endl;
		std::cin >> x;
		std::cin >> y;
		if( ((x3-x2) * (y-y3) - (y3-y2) * (x-x3)) > 0 )
			...
			{
	std::cout << "This is not convex polygon " << std::endl;
			break;		
			}
			temp++; 
		}
if(((x3-x2) * (y-y3) - (y3-y2) * (x-x3)) > 0){
	  if( ((x3-x2) * (y1-y3) - (y3-y2) * (x1-x3))>0 )
		...
		if( ((x1-x3) * (y2-y1) - (y1-y3) * (x2-x1)) > 0 )
				{
				std::cout << "This is convex polygon " << std::endl;
				}
			else 
				std::cout << "This is not convex polygon " << std::endl;
		}
