...
int main()
{
  ...
  for (int i=0; i<max_demical;++i)
	...
	while (j<(n-1))
	  {
	    if (number[j]==0 && number[j+1]==0 && number[n-1])
	      {
		counter=counter+1;
		j=n;
	      }
  ...
  delete number;
  cout << k <<"-ական համակարգի " << n << "-անիշ թվերի քանակն է՝ " << counter1 << endl;
  cout << "Երկու և ավելի անընդմեջ զրոներով թվերի քանակն է՝ " << counter << endl;
  cout << "Առանց երկու անընդմեջ զրոների թվերի քանակն է՝ " << counter1-counter << endl;
  return 0;
}
