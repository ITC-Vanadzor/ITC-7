...
int main()
{
  ...
  cout << "Երկու և ավելի անընդմեջ զրոներով թվերի քանակն է՝ " << (k-1)*(k*(n-2)-(n-3)) << endl;
  cout << "Առանց երկու անընդմեջ զրոների թվերի քանակն է՝ " << (k-1)*pow(k,n-1)-(k-1)*(n-2)*(n-1)/2 << endl;
  return 0;
}
